...
void PRF_SHA256( u8 * key, u16 key_length, u8 * sequence, u16 sequence_length, u8 * result ) {
    unsigned int size;
    HMAC(EVP_sha256(), key, key_length, sequence, sequence_length, result, &size);
}

void PRF_plus_SHA256( u8 iter, u8 * key, u16 key_length, u8 * sequence, u16 sequence_length, u8 * result ){    
    u16 prf_size = EVP_MD_size(EVP_sha256());
    u8 *temp = malloc(prf_size*sizeof(u8));
	...
	// Calculate T1 = prf(K, S | 0x01) = prf(K, new_sequence)
    PRF_SHA256(key, key_length, new_sequence, sequence_length + 1, temp);
    ...
    for (current_iter = 2; current_iter <= iter; current_iter++) {
        ...
        // Calculate T1 = prf(K, S | 0x01) = prf(K, new_sequence)
        PRF_SHA256(key, key_length, new_sequence, sequence_length + prf_size + 1, temp);
        // Insert into result
        memcpy(&result[ (current_iter - 1) * prf_size], temp, prf_size);
    }
