...
static void radius_client_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	if (radius->msgs) {
		if (first < now.sec)
			first = now.sec;
		/*Basically here we should add this time to the alarm list. In the end, this is adding a new job to the alarm list*/
...
static void radius_client_update_timeout(struct radius_client_data *radius)
{
	...
	/*Rafa:This should remove a node in the alarm list*/
	...
	if (first < now.sec)
		first = now.sec;
	/*Rafa:This adds a new alarm to the alarm list*/
...
static void radius_client_list_add(struct radius_client_data *radius,
				   ...
				   RadiusType msg_type,
				   const u8 *shared_secret,
				   size_t shared_secret_len, const u8 *addr, void *session)
{
	struct radius_msg_list *entry, *prev;

	/*Rafa: This functions seems to add a set of messages that have to be retransmitted*/
	//if (eloop_terminated()) {
	...
	entry->session = session; //This is going to point to the session which provokes sending the radius request.
...
void radius_client_receive(struct radius_msg *msg, void *eloop_ctx, void *sock_ctx)
{
	...
	RadiusType msg_type = (RadiusType) *rtype;
	int len, roundtrip;
	...
	for (i = 0; i < num_handlers; i++) {
		...
		res = handlers[i].handler(msg, req->msg, req->shared_secret,
								  ...
								  handlers[i].data); /*Rafa: This function is assigned in radius_client_register*/
		switch (res) {
...
/**This function calls connect() to connecto with RADIUS server*/
static int
radius_change_server(struct radius_client_data *radius,
		     struct hostapd_radius_server *nserv,
		     struct hostapd_radius_server *oserv,
		     int sock, int sock6, int auth)
{
	...
	if (radius->msgs) {
		/*Here it seems the alarm is stopped and then it waits for sometime to retransmit most probably.
		 * 
		/*eloop_cancel_timeout(radius_client_timer, radius, NULL);
	...
	/*We DO NOT DO connect over an already initialized socket */
...
static void radius_retry_primary_timer(void *eloop_ctx, void *timeout_ctx)
{
	...
	/*This basically means: to add a new alarm to the alarms list processed by alarm thread*/
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
    ...
    /*Rafa: We must change this to add this socket to the select in the network manager thread*/
	/*if (radius->auth_serv_sock >= 0 &&
...
void radius_client_deinit(struct radius_client_data *radius)
{
	...
	/*Rafa: we should remove this socket from the list of sockets that the network manager thread has to read in the select*/
	/*if (radius->auth_serv_sock >= 0)
...
#endif /* CONFIG_IPV6 */
    /* Rafa: something should be done here related with alarm manager*/
